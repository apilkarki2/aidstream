...
class XmlValidator
{
    ...
    public function rulesForOtherIdentifier(array $activity)
    {
        ...
        foreach ($otherIdentifiers as $otherIdentifierIndex => $otherIdentifier) {
            ...
            $rules                                                = array_merge(
                $rules,
                $this->rulesForOwnerOrg($otherIdentifier['owner_org'], $otherIdentifierBase)
            );
        }
    ...
    public function messagesForOtherIdentifier(array $activity)
    {
        ...
        foreach ($otherIdentifiers as $otherIdentifierIndex => $otherIdentifier) {
            ...
            $messages                                                         = array_merge(
                $messages,
                $this->messagesForOwnerOrg($otherIdentifier['owner_org'], $otherIdentifierBase)
            );
        }
    ...
    protected function rulesForContactInfo(array $activity)
    {
        ...
        foreach ($contacts as $contactInfoIndex => $contactInfo) {
            ...
            $rules                                       = array_merge(
                $rules,
                $this->getRulesForDepartment($contactInfo['department'], $contactInfoBase),
                $this->getRulesForOrganization($contactInfo['organization'], $contactInfoBase),
                $this->getRulesForPersonName($contactInfo['person_name'], $contactInfoBase),
                $this->getRulesForJobTitle($contactInfo['job_title'], $contactInfoBase),
                $this->getRulesForMailingAddress($contactInfo['mailing_address'], $contactInfoBase),
                $this->getRulesForEmail($contactInfo['email'], $contactInfoBase),
                $this->getRulesForWebsite($contactInfo['website'], $contactInfoBase)
            );
        }
    ...
    protected function messagesForContactInfo(array $activity)
    {
        ...
        foreach ($contacts as $contactInfoIndex => $contactInfo) {
            ...
            $messages                                          = array_merge(
                $messages,
                $this->getMessagesForDepartment($contactInfo['department'], $contactInfoBase),
                $this->getMessagesForOrganization($contactInfo['organization'], $contactInfoBase),
                $this->getMessagesForPersonName($contactInfo['person_name'], $contactInfoBase),
                $this->getMessagesForJobTitle($contactInfo['job_title'], $contactInfoBase),
                $this->getMessagesForMailingAddress($contactInfo['mailing_address'], $contactInfoBase),
                $this->getMessagesForEmail($contactInfo['email'], $contactInfoBase),
                $this->getMessagesForWebsite($contactInfo['website'], $contactInfoBase)
            );
        }
    ...
    protected function rulesForLocation(array $activity)
    {
        ...
        foreach ($locations as $locationIndex => $location) {
            ...
            $rules                                                           = array_merge(
                $rules,
                $this->getRulesForLocationId($location['location_id'], $locationBase),
                $this->getRulesForName($location['name'], $locationBase),
                $this->getRulesForLocationDescription($location['location_description'], $locationBase),
                $this->getRulesForActivityDescription($location['activity_description'], $locationBase),
                $this->getRulesForAdministrative($location['administrative'], $locationBase),
                $this->getRulesForPoint($location['point'], $locationBase)
            );
        }
    ...
    protected function messagesForLocation(array $activity)
    {
        ...
        foreach ($locations as $locationIndex => $location) {
            ...
            $messages                                                              = array_merge(
                $messages,
                $this->getMessagesForLocationId($location['location_id'], $locationBase),
                $this->getMessagesForName($location['name'], $locationBase),
                $this->getMessagesForLocationDescription($location['location_description'], $locationBase),
                $this->getMessagesForActivityDescription($location['activity_description'], $locationBase),
                $this->getMessagesForAdministrative($location['administrative'], $locationBase),
                $this->getMessagesForPoint($location['point'], $locationBase)
            );
        }
    ...
    public function messagesForCountryBudgetItems(array $activity)
    {
        ...
        foreach ($countryBudgetItems as $countryBudgetItemIndex => $countryBudgetItem) {
            ...
            $messages                                                                         = array_merge(
                $messages,
                $this->getBudgetItemMessages($countryBudgetItem['budget_item'], $countryBudgetItemBase, $code)
            );
        }
    ...
    public function getBudgetItemRules($budgetItems, $countryBudgetItemBase, $code, $countryBudgetItems)
    {
        ...
        foreach ($budgetItems as $budgetItemIndex => $budgetItem) {
            ...
            $rules                                            = array_merge(
                $rules,
                $this->getBudgetItemDescriptionRules($budgetItem['description'], $budgetItemBase)
            );
            $rules                                            = array_merge(
                $rules,
    ...
    public function getBudgetItemMessages($budgetItems, $countryBudgetItemBase, $code)
    {
        ...
        foreach ($budgetItems as $budgetItemIndex => $budgetItem) {
            ...
            $messages                                                          = array_merge(
                $messages,
                $this->getBudgetItemDescriptionMessages($budgetItem['description'], $budgetItemBase)
            );
        }
    ...
    public function rulesForHumanitarianScope(array $activity)
    {
        ...
        foreach ($humanitarianScopes as $humanitarianScopeIndex => $humanitarianScope) {
            ...
            $rules                                             = array_merge($rules, $this->factory->getRulesForNarrative($humanitarianScope['narrative'], $humanitarianScopeBase));
        }
    ...
    public function messagesForHumanitarianScope(array $activity)
    {
        ...
        foreach ($humanitarianScopes as $humanitarianScopeIndex => $humanitarianScope) {
            ...
            $messages                                                  = array_merge($messages, $this->factory->getMessagesForNarrative($humanitarianScope['narrative'], $humanitarianScopeForm));
        }
